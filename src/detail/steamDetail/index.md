...
```js
  const quarterData = FileAttachment("../../data/steamQuarter.csv").csv({typed: true});
```
    ...
    <div class="card">
      ...
      ${resize((width)=>
        Plot.plot({
          ...
          marks:[
            Plot.ruleY([0]),
            Plot.lineY(recentData,{x:"copyDate",y:"total",stroke:"year",
            title: d => `<strong>${d.date}</strong><br>数值: 3333<br>日期: ${d.date}`
            })
          ],
          x:{
            type:"time",
