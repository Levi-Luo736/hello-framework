...
const quarterData = mergedData.reduce((acc, item) => {
  ...
  if (itemQuarter==quarter) {
    acc.push({date:itemDate,
         ...
         quarter:itemQuarter,
         total:Math.round(Number(item.companyDiningHall)+Number(item.westernKitchen1)+Number(item.westernKitchen2)+Number(item.chineseKitchenEast2)+Number(item.chineseKitchenEast1)+Number(item.chineseKitchenWest2)+Number(item.chineseKitchenWest1)),
         companyDiningHall:item.companyDiningHall,
         westernKitchen1:item.westernKitchen1,
         westernKitchen2:item.westernKitchen2,
         chineseKitchenEast2:item.chineseKitchenEast2,
         chineseKitchenEast1:item.chineseKitchenEast1,
         chineseKitchenWest2:item.chineseKitchenWest2,
         chineseKitchenWest1:item.chineseKitchenWest1
    });
  }
  return acc;
}, []);
