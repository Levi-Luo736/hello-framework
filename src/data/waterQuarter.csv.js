...
async function load() {
   ...
   const curYear = yestarday.getFullYear();
...
const quarterData = mergedData.reduce((acc, item) => {
  ...
  if (itemQuarter==quarter) {
    acc.push({date:itemDate,
         ...
         quarter:itemQuarter,
         total:Math.round(Number(item.domesticWater)+Number(item.fireWater)),
         lifeWater:item.domesticWater,
         fireWater:item.fireWater
    });
  }
  return acc;
}, []);
